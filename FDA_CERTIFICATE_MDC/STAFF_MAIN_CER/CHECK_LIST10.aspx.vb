...
Public Class CHECK_LIST10
    ...
    Private Sub RadGrid1_ItemCommand(sender As Object, e As Telerik.Web.UI.GridCommandEventArgs) Handles RadGrid1.ItemCommand
        If TypeOf e.Item Is GridDataItem Then
            ...
            If e.CommandName = "Sel" Then
                ...
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "clientScript", "Popups('FRM_POPUP_STAFF_CONFRIME.aspx?ida=" & 
                                                        ida_sel & "&P_ID=" & _P_ID & "&TR_ID=" & tr_id & "');", True)
        ...
        Try
            If txt_rcvno.Text.Length > 0 And txt_ref_code.Text.Length > 0 Then
                Dim dataSource = From cer In db.MDC_CERs Join s In db.MAS_STATUS On cer.STATUS_ID Equals s.STATUS_ID
                                 ...
                                 cer.country_no, cer.CITIZEN_ID, cer.CITIZEN_ID_UPLOAD,
            ...
            Else
                ...
                ElseIf txt_namecom.Text.Length > 0 Then
                    Dim dataSource = From cer In db.MDC_CERs Join s In db.MAS_STATUS On cer.STATUS_ID Equals s.STATUS_ID
                                     Where cer.company_name.Contains(txt_namecom.Text) And s.STATUS_GROUP = 1 And cer.STATUS_PAY = 1
                                     Order By s.rid Ascending, cer.IDA Descending
                                     Select cer.IDA, cer.rcvno,
                                     cer.rcvdate, cer.expdate, cer.cer_type, cer.for_type, cer.TR_ID,
                                     cer.country_no, cer.CITIZEN_ID, cer.CITIZEN_ID_UPLOAD,
                    ...
                    '    'เอาไว้ใส่สำหรับช่องค้นหาผลิตภัณฑ์
                    'ElseIf txt_product.Text.Length > 0 Then
                    'Dim dataSource = From cer In db.MDC_CER_PRODUCTs Join s In db.MAS_STATUS On cer.Product_Name Equals s.STATUS_ID
                    '                 Where cer.company_name.Contains(txt_product.Text) And s.STATUS_GROUP = 1 And cer.STATUS_PAY = 1
                    '                 Order By s.rid Ascending, cer.IDA Descending
                    '                 Select cer.IDA, cer.rcvno,
                    '                 cer.rcvdate, cer.expdate, cer.cer_type, cer.for_type, cer.TR_ID,
                    '                 cer.country_no, cer.CITIZEN_ID, cer.CITIZEN_ID_UPLOAD,
    ...
    End Sub
    'ตัวนี้จะทำการดึงข้อมูลของ TB MDC_CER ทั้งหมด
    Private Sub RadGrid1_NeedDataSource(sender As Object, e As GridNeedDataSourceEventArgs) Handles RadGrid1.NeedDataSource
        'RadGrid1.DataSource = dataSource
        RadGrid1.DataSource = bao.SP_MDC_CER_SHOW_STAFF_CHECK_LIST10()
    End Sub
